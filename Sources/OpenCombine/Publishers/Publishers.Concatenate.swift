...
extension Publishers {
        ...
        where Prefix.Failure == Suffix.Failure, Prefix.Output == Suffix.Output
    {
        ...
        {
            prefix.subscribe(Inner(downstream: subscriber, suffix: suffix))
        }
    }
}
...
extension Publishers.Concatenate {
        ...
        where Downstream.Input == Suffix.Output, Downstream.Failure == Suffix.Failure
    {
        ...
        // TODO: Test that this deadlocks so that we know when they've fixed it.
        private let downstreamLock = UnfairLock.allocate() // 0xB8 (184)
